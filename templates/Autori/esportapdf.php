...
$html .= '<th>cognome</th>';
$html .= '<th>note</th>';
$html .= '<th>genere_id</th>';
$html .= '</tr></thead>';
 foreach ($autori as $autore) : 
 $html .= '<tr>';
        $html.='<td>' . $this->Number->format($autore->id).'</td>';
        $html.='<td>' . h($autore->nome).'</td>';
        $html.='<td>' . h($autore->cognome).'</td>';
        $html.='<td>' . h($autore->note).'</td>';
 $autore->has('genere') ? $html.= '<td>' .$autore->genere->title .'</td>' : $html.='<td></td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('autori');
	$this->Pdf->core->SetSubject('autori');
	$this->Pdf->core->SetKeywords('autori');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'autori', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_autori.pdf', 'D');      
