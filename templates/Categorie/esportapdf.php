...
$html .= '</tr></thead>';
 ...
 $html .= '<tr>';
        $html.='<td>' . $this->Number->format($categoria->id).'</td>';
        $html.='<td>' . h($categoria->title).'</td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('categorie');
	$this->Pdf->core->SetSubject('categorie');
	$this->Pdf->core->SetKeywords('categorie');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'categorie', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_categorie.pdf', 'D');      
