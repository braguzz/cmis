...
$html .= '</tr></thead>';
 ...
 $html .= '<tr>';
        $html.='<td>' . $this->Number->format($lingua->id).'</td>';
        $html.='<td>' . h($lingua->title).'</td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('lingue');
	$this->Pdf->core->SetSubject('lingue');
	$this->Pdf->core->SetKeywords('lingue');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'lingue', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_lingue.pdf', 'D');      
