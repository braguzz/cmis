...
$html .= '<th>CODPROG</th>';
$html .= '<th>VERSIONE</th>';
$html .= '<th>DATANUCLEO</th>';
$html .= '<th>FLAGB</th>';
$html .= '<th>COD_TIPOCRITICITA_NV</th>';
$html .= '<th>COD_TIPOSOLUZIONE_NV</th>';
$html .= '<th>DATACOMASS</th>';
$html .= '<th>SOL_GR_A</th>';
$html .= '<th>DATA_SOL_GR</th>';
$html .= '<th>CRITICITA_NDV</th>';
$html .= '</tr></thead>';
 foreach ($prove as $prova) : 
 $html .= '<tr>';
        $html.='<td>' . $this->Number->format($prova->CODPROG).'</td>';
        $html.='<td>' . h($prova->VERSIONE).'</td>';
        $html.='<td>' . h($prova->DATANUCLEO).'</td>';
        $html.='<td>' . h($prova->FLAGB).'</td>';
        $html.='<td>' . h($prova->COD_TIPOCRITICITA_NV).'</td>';
        $html.='<td>' . h($prova->COD_TIPOSOLUZIONE_NV).'</td>';
        $html.='<td>' . h($prova->DATACOMASS).'</td>';
        $html.='<td>' . h($prova->SOL_GR_A).'</td>';
        $html.='<td>' . h($prova->DATA_SOL_GR).'</td>';
        $html.='<td>' . h($prova->CRITICITA_NDV).'</td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('prove');
	$this->Pdf->core->SetSubject('prove');
	$this->Pdf->core->SetKeywords('prove');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'prove', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_prove.pdf', 'D');      
