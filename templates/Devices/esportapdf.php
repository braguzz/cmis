...
$html .= '<thead><tr>';
$html .= '<th>id</th>';
$html .= '<th>utenza_imei</th>';
$html .= '<th>devmodel_id</th>';
$html .= '<th>data_carico</th>';
$html .= '<th>data_scarico</th>';
$html .= '<th>created</th>';
$html .= '<th>modified</th>';
$html .= '<th>mac</th>';
$html .= '</tr></thead>';
 ...
 $html .= '<tr>';
         $html.='<td>' . h($device->id).'</td>';
        $html.='<td>' . h($device->utenza_imei).'</td>';
 $device->has('devmodel') ? $html.= '<td>' .$device->devmodel->title .'</td>' : $html.='<td></td>';
        $html.='<td>' . h($device->data_carico).'</td>';
        $html.='<td>' . h($device->data_scarico).'</td>';
        $html.='<td>' . h($device->created).'</td>';
        $html.='<td>' . h($device->modified).'</td>';
        $html.='<td>' . h($device->mac).'</td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('devices');
	$this->Pdf->core->SetSubject('devices');
	$this->Pdf->core->SetKeywords('devices');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'devices', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_devices.pdf', 'D');      
