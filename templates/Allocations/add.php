...
<h1 class="page-header">Aggiungi Allocation</h1>
 </div>  
 </div>
</div>
    ...
    <fieldset>
        <?php
          //  echo $this->Form->control('device_id', ['options' => $devices, 'empty' => true]);
                                               if (($return['returncontroller'])=='devices')  echo $this->Form->hidden('device_id', array('hiddenField' => true, 'value'=> $return['returncontrollerid']));
                    else {
                                               
                       $myTemplates = ['select' => '   <div class="input-group"><select name="{{name}}"{{attrs}}>{{content}}</select>
                                                <div class="input-group-append">
                                                <button id="deviceButtonAdd" class="btn btn-outline-secondary bi-plus-lg" type="button"></button>
                                                </div></div>'];
...
<script>
     $(document).ready(function () {
        $('#deviceButtonAdd').click({nome: 'Devices', returncontroller: 'allocations'}, Aggiungi);            doAcSelect2('device_id', 'Devices' , 'id', 3);  
        });
</script>
           
<!-- Bottoni per apertura delle finestre modali     -->  
<button id='buttonmodalDevices' type='button' class='d-none btn btn-primary btn-lg' data-toggle='modal' data-target='#modalDevices'>
</button>
<!-- Contenuto delle finestre modali (viene riempito via JS)-->
<div class='content modal fade' id='modalDevices' tabindex='-1' role='dialog' aria-labelledby='myModalLabel'>
</div>
 ...
 <?php
                  ...
                  //  echo $this->Form->control('owner_id', ['options' => $owners, 'empty' => true]);
                                               if (($return['returncontroller'])=='owners')  echo $this->Form->hidden('owner_id', array('hiddenField' => true, 'value'=> $return['returncontrollerid']));
                    else {
                                               
                       $myTemplates = ['select' => '   <div class="input-group"><select name="{{name}}"{{attrs}}>{{content}}</select>
                                                <div class="input-group-append">
                                                <button id="ownerButtonAdd" class="btn btn-outline-secondary bi-plus-lg" type="button"></button>
                                                </div></div>'];
...
<script>
     $(document).ready(function () {
        $('#ownerButtonAdd').click({nome: 'Owners', returncontroller: 'allocations'}, Aggiungi);            doAcSelect2('owner_id', 'Owners' , 'name', 3);  
        });
</script>
           
<!-- Bottoni per apertura delle finestre modali     -->  
<button id='buttonmodalOwners' type='button' class='d-none btn btn-primary btn-lg' data-toggle='modal' data-target='#modalOwners'>
</button>
<!-- Contenuto delle finestre modali (viene riempito via JS)-->
<div class='content modal fade' id='modalOwners' tabindex='-1' role='dialog' aria-labelledby='myModalLabel'>
</div>
