...
$html .= '</tr></thead>';
 ...
 $html .= '<tr>';
        $html.='<td>' . $this->Number->format($disco->id).'</td>';
        $html.='<td>' . h($disco->title).'</td>';
        $html.='<td>' . h($disco->descrizione).'</td>';
 $disco->has('lingua') ? $html.= '<td>' .$disco->lingua->title .'</td>' : $html.='<td></td>';
 $disco->has('autore') ? $html.= '<td>' .$disco->autore->id .'</td>' : $html.='<td></td>';
        $html.='<td>' . h($disco->data).'</td>';
        $html.='<td>' . h($disco->datetime).'</td>';
        $html.='<td>' . h($disco->created).'</td>';
        $html.='<td>' . h($disco->modified).'</td>';
       $html.='<td>' . $this->Number->format($disco->intero).'</td>';
        $html.='<td>' . h($disco->booleano).'</td>';
       $html.='<td>' . $this->Number->format($disco->decimale).'</td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('dischi');
	$this->Pdf->core->SetSubject('dischi');
	$this->Pdf->core->SetKeywords('dischi');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'dischi', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_dischi.pdf', 'D');      
