...
EOF;
//$html .= '<h1>categorieLibri</h1>';
$html .= '<table  cellspacing="0" cellpadding="1" border="1">';
$html .= '<thead><tr>';
$html .= '<th>id</th>';
$html .= '<th>libro_id</th>';
$html .= '<th>categoria_id</th>';
$html .= '</tr></thead>';
 foreach ($categorieLibri as $categorieLibro) : 
 $html .= '<tr>';
        $html.='<td>' . $this->Number->format($categorieLibro->id).'</td>';
 $categorieLibro->has('libro') ? $html.= '<td>' .$categorieLibro->libro->id .'</td>' : $html.='<td></td>';
 $categorieLibro->has('categoria') ? $html.= '<td>' .$categorieLibro->categoria->title .'</td>' : $html.='<td></td>';
...
$html .= '</table>';
	...
	$this->Pdf->core->SetTitle('categorieLibri');
	$this->Pdf->core->SetSubject('categorieLibri');
	$this->Pdf->core->SetKeywords('categorieLibri');
	// set default header data
	$this->Pdf->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'categorieLibri', '');
	...
	$this->Pdf->core->writeHTML($html, true, false, false, false, '');
        ob_end_clean();
	$this->Pdf->core->Output('Export_categorieLibri.pdf', 'D');      
