...
var infoDocTab = {
  title: 'Info',
  html: 'Séléctionnez un document',
  itemId: 'infotab',
  tpl: new Ext.Template([
    '<p>Nom : {cmis-name}</p>',
    '<p>Date de création : {cmis-creationDate}</p>',
    '<p>Créé par : {cmis-createdBy}</p>'
  ].join(''), {compiled: true})
};
...
TYPO3.AtolCmis.openDial = function(hn){
  ...
  var validate = function (btn){
    if(TYPO3.AtolCmis.Data.type=='cmis:folder'){
      document.editform[hn].value=TYPO3.AtolCmis.Data.name+';;_tx_;;'+TYPO3.AtolCmis.Data.objectId;
      document.editform[hn+'_0'].value=TYPO3.AtolCmis.Data.name;
      close();
    }
    else{
      alert("Veuillez sélectionner un dossier afin de valider.");
    }
  }
