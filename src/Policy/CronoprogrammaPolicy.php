...
use App\Model\Entity\Cronoprogramma;
use Authorization\IdentityInterface;
use Authorization\Policy\BeforePolicyInterface;
use Cake\Core\Configure;

/**
 * Cronoprogramma policy
 */
 class CronoprogrammaPolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Cronoprogramma->canRead) && $user->role->data_config->Cronoprogramma->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Cronoprogramma->canUpdate) && $user->role->data_config->Cronoprogramma->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Cronoprogramma->canCreate) && $user->role->data_config->Cronoprogramma->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Cronoprogramma->canDelete) && $user->role->data_config->Cronoprogramma->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
    ...
    if (isset( $user->role->data_config->Cronoprogramma->$canatc ))
    {     
    return  filter_var($user->role->data_config->Cronoprogramma->$canatc, FILTER_VALIDATE_BOOLEAN);
    }      
 ...
 //END PB  
    ...
    /**
     * Check if $user can add Cronoprogramma
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Cronoprogramma $cronoprogramma
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Cronoprogramma $cronoprogramma)
    {
    }

    /**
     * Check if $user can edit Cronoprogramma
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Cronoprogramma $cronoprogramma
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Cronoprogramma $cronoprogramma)
    {
    }

    /**
     * Check if $user can delete Cronoprogramma
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Cronoprogramma $cronoprogramma
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Cronoprogramma $cronoprogramma)
    {
    }

    /**
     * Check if $user can view Cronoprogramma
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Cronoprogramma $cronoprogramma
     * @return bool
     */
    public function canView(IdentityInterface $user, Cronoprogramma $cronoprogramma)
    {
    }
}
