 ...
 * Autore policy
 */
 class AutorePolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Autore->canRead) && $user->role->data_config->Autore->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Autore->canUpdate) && $user->role->data_config->Autore->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Autore->canCreate) && $user->role->data_config->Autore->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Autore->canDelete) && $user->role->data_config->Autore->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
 ...
 //END PB  
    ...
    /**
     * Check if $user can add Autore
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Autore $autore
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Autore $autore)
    {
    }

    /**
     * Check if $user can edit Autore
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Autore $autore
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Autore $autore)
    {
    }

    /**
     * Check if $user can delete Autore
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Autore $autore
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Autore $autore)
    {
    }

    /**
     * Check if $user can view Autore
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Autore $autore
     * @return bool
     */
    public function canView(IdentityInterface $user, Autore $autore)
    {
    }
}
