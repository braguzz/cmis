 ...
 * Allocation policy
 */
 class AllocationPolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Allocation->canRead) && $user->role->data_config->Allocation->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Allocation->canUpdate) && $user->role->data_config->Allocation->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Allocation->canCreate) && $user->role->data_config->Allocation->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Allocation->canDelete) && $user->role->data_config->Allocation->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
    ...
    if (isset( $user->role->data_config->Allocation->$canatc ))
    {     
    return  filter_var($user->role->data_config->Allocation->$canatc, FILTER_VALIDATE_BOOLEAN);
    }      
 ...
 //END PB  
     ...
     * @param \App\Model\Entity\Allocation $allocation
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Allocation $allocation)
    {
    }
     ...
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Allocation $allocation)
    {
    }
     ...
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Allocation $allocation)
    {
    }

    /**
     * Check if $user can view Allocation
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Allocation $allocation
     * @return bool
     */
    public function canView(IdentityInterface $user, Allocation $allocation)
    {
    }
}
