...
use App\Model\Entity\Intervento;
 ...
 * Intervento policy
 */
 class InterventoPolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Intervento->canRead) && $user->role->data_config->Intervento->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Intervento->canUpdate) && $user->role->data_config->Intervento->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Intervento->canCreate) && $user->role->data_config->Intervento->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Intervento->canDelete) && $user->role->data_config->Intervento->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
    ...
    if (isset( $user->role->data_config->Intervento->$canatc ))
    {     
    return  filter_var($user->role->data_config->Intervento->$canatc, FILTER_VALIDATE_BOOLEAN);
    }      
 ...
 //END PB  
    ...
    /**
     * Check if $user can add Intervento
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Intervento $intervento
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Intervento $intervento)
    {
    }

    /**
     * Check if $user can edit Intervento
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Intervento $intervento
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Intervento $intervento)
    {
    }

    /**
     * Check if $user can delete Intervento
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Intervento $intervento
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Intervento $intervento)
    {
    }

    /**
     * Check if $user can view Intervento
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Intervento $intervento
     * @return bool
     */
    public function canView(IdentityInterface $user, Intervento $intervento)
    {
    }
}
