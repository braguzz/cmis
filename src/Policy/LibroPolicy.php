 ...
 * Libro policy
 */
 class LibroPolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Libro->canRead) && $user->role->data_config->Libro->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Libro->canUpdate) && $user->role->data_config->Libro->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Libro->canCreate) && $user->role->data_config->Libro->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Libro->canDelete) && $user->role->data_config->Libro->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
 ...
 //END PB  
    ...
    /**
     * Check if $user can add Libro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Libro $libro
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Libro $libro)
    {
    }

    /**
     * Check if $user can edit Libro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Libro $libro
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Libro $libro)
    {
    }

    /**
     * Check if $user can delete Libro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Libro $libro
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Libro $libro)
    {
    }

    /**
     * Check if $user can view Libro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Libro $libro
     * @return bool
     */
    public function canView(IdentityInterface $user, Libro $libro)
    {
    }
}
