 ...
 * Lingue policy
 */
 class LinguePolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Lingue->canRead) && $user->role->data_config->Lingue->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Lingue->canUpdate) && $user->role->data_config->Lingue->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Lingue->canCreate) && $user->role->data_config->Lingue->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Lingue->canDelete) && $user->role->data_config->Lingue->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
 ...
 //END PB  
    ...
    /**
     * Check if $user can add Lingue
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Lingue $lingue
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Lingue $lingue)
    {
    }

    /**
     * Check if $user can edit Lingue
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Lingue $lingue
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Lingue $lingue)
    {
    }

    /**
     * Check if $user can delete Lingue
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Lingue $lingue
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Lingue $lingue)
    {
    }

    /**
     * Check if $user can view Lingue
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Lingue $lingue
     * @return bool
     */
    public function canView(IdentityInterface $user, Lingue $lingue)
    {
    }
}
