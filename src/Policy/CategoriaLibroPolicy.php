...
use App\Model\Entity\CategoriaLibro;
use Authorization\IdentityInterface;
use Authorization\Policy\BeforePolicyInterface;
use Cake\Core\Configure;

/**
 * CategoriaLibro policy
 */
 class CategoriaLibroPolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->CategoriaLibro->canRead) && $user->role->data_config->CategoriaLibro->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->CategoriaLibro->canUpdate) && $user->role->data_config->CategoriaLibro->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->CategoriaLibro->canCreate) && $user->role->data_config->CategoriaLibro->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->CategoriaLibro->canDelete) && $user->role->data_config->CategoriaLibro->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
 ...
 //END PB  
    ...
    /**
     * Check if $user can add CategoriaLibro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\CategoriaLibro $categoriaLibro
     * @return bool
     */
    public function canAdd(IdentityInterface $user, CategoriaLibro $categoriaLibro)
    {
    }

    /**
     * Check if $user can edit CategoriaLibro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\CategoriaLibro $categoriaLibro
     * @return bool
     */
    public function canEdit(IdentityInterface $user, CategoriaLibro $categoriaLibro)
    {
    }

    /**
     * Check if $user can delete CategoriaLibro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\CategoriaLibro $categoriaLibro
     * @return bool
     */
    public function canDelete(IdentityInterface $user, CategoriaLibro $categoriaLibro)
    {
    }

    /**
     * Check if $user can view CategoriaLibro
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\CategoriaLibro $categoriaLibro
     * @return bool
     */
    public function canView(IdentityInterface $user, CategoriaLibro $categoriaLibro)
    {
    }
}
