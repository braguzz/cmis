 ...
 * Disco policy
 */
 class DiscoPolicy implements BeforePolicyInterface
 {
 ...
 {    
    ...
    if (isset($user->role->data_config->all->canDoAll))
    {
       ...
       //cerca di vedere se ci sono autorizzazioni globali relative a questa tabella definite in regtoscconf- DA TESTARE       
             if (isset($user->role->data_config->Disco->canRead) && $user->role->data_config->Disco->canRead)
            {        
               if (in_array ( $action, $autorizzazioniMap['Read'] )) return true;
            } 
       if (isset($user->role->data_config->Disco->canUpdate) && $user->role->data_config->Disco->canUpdate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Update'] )) return true;
            } 
        if (isset($user->role->data_config->Disco->canCreate) && $user->role->data_config->Disco->canCreate)
            {        
               if (in_array ( $action, $autorizzazioniMap['Create'] )) return true;
            }     
        if (isset($user->role->data_config->Disco->canDelete) && $user->role->data_config->Disco->canDelete)
            {        
               if (in_array ( $action, $autorizzazioniMap['Delete'] )) return true;
            }   
 ...
 //END PB  
    ...
    /**
     * Check if $user can add Disco
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Disco $disco
     * @return bool
     */
    public function canAdd(IdentityInterface $user, Disco $disco)
    {
    }

    /**
     * Check if $user can edit Disco
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Disco $disco
     * @return bool
     */
    public function canEdit(IdentityInterface $user, Disco $disco)
    {
    }

    /**
     * Check if $user can delete Disco
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Disco $disco
     * @return bool
     */
    public function canDelete(IdentityInterface $user, Disco $disco)
    {
    }

    /**
     * Check if $user can view Disco
     *
     * @param \Authorization\IdentityInterface $user The user.
     * @param \App\Model\Entity\Disco $disco
     * @return bool
     */
    public function canView(IdentityInterface $user, Disco $disco)
    {
    }
}
